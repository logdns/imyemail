...
测试进程使用固定本地主机标识；仅 CPU 计时压力程序使用固定 CPU 亲和性，网络并发测试保留正常的 CPU 调度。临时文件目录位于仅供构建使用的 tmpfs，避免共享磁盘等待干扰计时。每项测试的耗时和日志保留在源码构建目录，超时后仍可读取；CPU 压力测试额外记录用户态、内核态计时与资源限制，帮助区分共享运行器耗时波动和真正卡住，失败仍阻止发布。这些设置仅用于构建自测，不进入运行镜像，也不改变邮件协议回归使用的真实 Docker 数据卷。

上游顶层 `check-local` 会在运行测试前删除 `.test`，因此不能提前创建 tmpfs 链接。测试包装器在清理完成后原子建立链接，并检查其目标和实际文件系统类型；若它变成普通目录，立即失败，避免静默退回共享磁盘。该安排不修改或跳过上游清理逻辑及测试断言。
