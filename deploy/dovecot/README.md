...
上游自测以低权限用户执行，并在只保留回环接口的构建网络中运行：保留文件权限断言、TCP 和 Unix Socket 测试，同时避免测试使用的私网地址访问宿主局域网或被虚拟机代理伪造连接。普通测试程序限时五分钟，CPU 压力测试单独限时十五分钟：原生共享运行器的进程诊断显示，五分钟时仍持续推进并已通过 7/9 子项，但 `RLIMIT_CPU` 触发存在滞后。延长的只是该压力程序的墙钟上限，不修改上游 CPU 时间断言；超时仍即构建失败，不会跳过或按成功处理。许可证和来源信息另行复制到运行镜像，避免 Debian slim 的文档包排除规则将其省略。

测试进程使用固定本地主机标识；仅 CPU 计时压力程序使用固定 CPU 亲和性，网络并发测试保留正常的 CPU 调度。已编译源码从只读快照复制到构建专用 tmpfs，保留原始绝对路径，避免共享磁盘等待干扰计时。测试执行期间的日志位于该内存目录，失败时输出到构建日志；CPU 压力测试还输出用户态、内核态计时与资源限制，成功时也保留这份诊断输出。测试目录在构建步骤结束时释放，不进入运行镜像，也不改变邮件协议回归使用的真实 Docker 数据卷。

上游顶层 `check-local` 会在运行测试前删除 `.test`，而路径测试要求真实路径与编译路径一致，因此不能用指向其他目录的符号链接代替测试目录。整个源码测试目录放在 tmpfs 后，`.test` 可正常清理和重建；脚本强制检查实际文件系统类型，不修改或跳过上游清理逻辑及测试断言。
