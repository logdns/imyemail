...
测试进程使用固定本地主机标识；仅 CPU 计时压力程序使用固定 CPU 亲和性，网络并发测试保留正常的 CPU 调度。临时文件目录位于仅供构建使用的 tmpfs，避免共享磁盘等待干扰计时。每项测试的耗时和日志保留在源码构建目录，超时后仍可读取。这些设置仅用于构建自测，不进入运行镜像，也不改变邮件协议回归使用的真实 Docker 数据卷。
