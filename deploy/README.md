...
本文说明生产环境单容器部署、可选多容器调试部署、证书、镜像与常见故障。系统架构和组件边界见 [架构说明](../docs/ARCHITECTURE.md)，完整产品能力见 [功能说明](../docs/FEATURES.md)，版本变化见 [更新日志](../CHANGELOG.md)。
...
网站名称、浏览器标题和全站默认语言可在后台“系统设置 → 基础”修改，前后台模板可在“系统设置 → 界面模板”切换；`IMYEMAIL_SITE_NAME`、`IMYEMAIL_SITE_TITLE`、`IMYEMAIL_DEFAULT_LANGUAGE`、`IMYEMAIL_UI_TEMPLATE` 仅提供首次启动默认值，后台保存后以数据库设置为准。语言只支持 `zh-CN`、`zh-TW` 和 `en`，模板只支持 `imyemaildefault`、`imyemailcloud`、`imyemail-cloud-sy`、`imyemail-vbena` 和 `imyemail-cloud-byte`。新增模板的配置与回滚说明见 [界面模板](../docs/UI-TEMPLATES.md)。
...
`v1.3.25` 的默认镜像基于 Debian 13，包含 Postfix 3.10.13、Dovecot 2.4.5 和 Rspamd 4.1.5；Gateway 使用 Nginx 1.30，Operator 使用 Alpine 3.24，更新器固定 Watchtower 1.21.0。Postfix 采用 Debian 安全维护包；Dovecot 使用签名、指纹与 SHA-256 固定的上游源码，在原生 amd64/arm64 CI 中验证完整协议链路。两个镜像复用源码脚本，说明及许可证见 [Dovecot 构建与验证](dovecot/README.md)。软件包层按发布版本刷新安全更新，禁止在运行中的容器内手工替换二进制。升级前备份完整 Maildir；2.4.5 的线程索引修复会按需重建索引，回滚注意事项见 [运维手册](../docs/OPERATIONS.md)。
