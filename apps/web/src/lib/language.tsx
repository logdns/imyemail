...
export function useLanguage() {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener("imyemail:language", sync)
    return () => {
      window.removeEventListener("storage", sync)
      window.removeEventListener("imyemail:language", sync)
    }
  }, [])
...
const templateTranslations: { pattern: RegExp; "zh-TW": string; en: string }[] = [
  ...
  { pattern: /^IMAP：每 1 分钟 最多 (.+) 次请求$/, "zh-TW": "IMAP：每分鐘最多 $1 次請求", en: "IMAP: up to $1 requests per minute" },
  { pattern: /^POP3：每 1 分钟 最多 (.+) 次请求$/, "zh-TW": "POP3：每分鐘最多 $1 次請求", en: "POP3: up to $1 requests per minute" },
