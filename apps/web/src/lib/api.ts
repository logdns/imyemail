...
export const api = {
  ...
  aiMail: (payload: AIMailInput, signal: AbortSignal, message?: { id: string; externalAccountId?: string }) => {
    const path = !message ? "/api/mail/ai/compose" : message.externalAccountId
      ? `/api/mail/external-accounts/${encodeURIComponent(message.externalAccountId)}/messages/${encodeURIComponent(message.id)}/ai`
      : `/api/mail/messages/${encodeURIComponent(message.id)}/ai`
    return request<AIMailResult>(path, { method: "POST", body: JSON.stringify(payload), signal, timeoutMs: 50_000 })
  },
  ...
  setupTwoFactor: () => request<{ secret: string; otpauthUrl: string; serverTime: string }>("/api/me/2fa/setup", { method: "POST" }),
  enableTwoFactor: (code: string) => request<{ user: User; recoveryCodes: string[] }>("/api/me/2fa/enable", { method: "POST", body: JSON.stringify({ code }) }),
  disableTwoFactor: (code: string) => request<{ user: User }>("/api/me/2fa/disable", { method: "POST", body: JSON.stringify({ code }) }),
  createMailboxAppPassword: ({ id, code }: { id: string; code: string }) => request<{ password: string; createdAt: string }>(`/api/me/mailboxes/${id}/app-password`, { method: "POST", body: JSON.stringify({ code }) }),
  deleteMailboxAppPassword: (id: string) => request<{ ok: boolean }>(`/api/me/mailboxes/${id}/app-password`, { method: "DELETE" }),
  ...
  resetUserTwoFactor: (id: string) => request<{ ok: boolean }>(`/api/admin/users/${id}/2fa/reset`, { method: "POST" }),
  ...
  createMailbox: (payload: { domainId: string; localPart: string; displayName: string; password: string; quotaMb: number; attachmentLimitMb: number; role: "admin" | "user"; ownerLoginName?: string; ownerEmail?: string; userId?: string }) => request<Mailbox>("/api/admin/mailboxes", { method: "POST", body: JSON.stringify(payload) }),
  updateMailbox: (id: string, payload: { userId: string; displayName: string; quotaMb: number; attachmentLimitMb: number; status: string }) => request<Mailbox>(`/api/admin/mailboxes/${id}`, { method: "POST", body: JSON.stringify(payload) }),
  ...
  deleteSystemRollback: () => request<SystemRollbackDeleteResult>("/api/admin/system/rollback", { method: "DELETE", body: JSON.stringify({ confirm: true }) }),
  ...
  clearAnnouncement: () => request<{ ok: boolean }>("/api/admin/announcements/current", { method: "DELETE" }),
