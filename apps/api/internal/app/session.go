...
func (a *App) issueSession(w http.ResponseWriter, r *http.Request, userID string) error {
	token := randomToken()
	sessionID := newID("ses")
	expires := a.now().UTC().Add(time.Duration(a.configSnapshot().SessionTTLHours) * time.Hour)
	...
	http.SetCookie(w, &http.Cookie{
		Name:     a.configSnapshot().CookieName,
		Value:    token,
		Path:     "/",
		Expires:  expires,
		...
		Secure:   !a.configSnapshot().AllowInsecureHTTP,
	})
	return nil
}
