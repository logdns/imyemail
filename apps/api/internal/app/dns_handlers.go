...
func (a *App) dnsRecordsFor(d *Domain) []DNSRecord {
	...
	host := strings.TrimSuffix(a.configSnapshot().PublicHostname, ".") + "."
	return []DNSRecord{
...
func (a *App) checkDNS(ctx context.Context, d *Domain) DNSCheckResult {
	...
	if err != nil || len(mx) == 0 {
		...
		for _, item := range mx {
			...
			if strings.EqualFold(strings.TrimSuffix(item.Host, "."), strings.TrimSuffix(a.configSnapshot().PublicHostname, ".")) {
				ok = true
			}
		}
