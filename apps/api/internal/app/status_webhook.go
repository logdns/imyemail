...
func (a *App) enqueueStatusWebhook(ctx context.Context, db dbExecutor, eventKey, eventType, mailboxID string, data any) error {
	if strings.TrimSpace(a.configSnapshot().StatusWebhookURL) == "" {
		return nil
	}
	now := a.now().UTC()
	id := newID("whk")
...
func (a *App) deliverStatusWebhook(ctx context.Context, eventID string, payload []byte) error {
	...
	mac := hmac.New(sha256.New, []byte(a.configSnapshot().StatusWebhookSecret))
	...
	req.Header.Set("User-Agent", "imyemail-webhook/1.0")
	req.Header.Set("X-imyemail-Webhook-Id", eventID)
	req.Header.Set("X-imyemail-Timestamp", timestamp)
	req.Header.Set("X-imyemail-Signature", "sha256="+hex.EncodeToString(mac.Sum(nil)))
	client := &http.Client{
...
func (a *App) validatedStatusWebhookURL(ctx context.Context) (*url.URL, error) {
	if strings.TrimSpace(a.configSnapshot().StatusWebhookSecret) == "" {
		return nil, errors.New("IMYEMAIL_STATUS_WEBHOOK_SECRET is required")
	}
	target, err := url.Parse(strings.TrimSpace(a.configSnapshot().StatusWebhookURL))
	if err != nil || target.Hostname() == "" || target.User != nil || target.Fragment != "" {
		return nil, errors.New("invalid status webhook URL")
	}
	if target.Scheme != "https" && !(a.configSnapshot().StatusWebhookAllowPrivateHosts && target.Scheme == "http") {
		return nil, errors.New("status webhook URL must use HTTPS")
	}
	if !a.configSnapshot().StatusWebhookAllowPrivateHosts {
