...
- `v1.3.25` (unreleased): adds the Arco Design-inspired `imyemail-cloud-byte` template across authentication, Webmail, profiles, and administration, with three languages, light/dark themes, and responsive layouts; patches editor and build dependency advisories. Publication is blocked on Dovecot security fixes. See [UI templates](docs/UI-TEMPLATES.md).
