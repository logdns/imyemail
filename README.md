...
| 管理后台 | 用户与权限组、域名与 DNS 检测、邮件健康评分、邮件收发与账户使用统计、邮箱与别名、邮件审计、发送队列、反馈工单、全域公告、三语言默认设置、界面模板和系统设置 |
...
- `v1.3.25`（未发布）：新增 Arco Design 风格的 `imyemail-cloud-byte` 前后台模板，支持三语言、浅深色、移动端与宽屏；修补编辑器依赖，采用经签名和哈希校验的 Dovecot 2.4.5。候选验证已通过，但主分支 amd64 CPU 压力测试重复超时，正式发布仍被门禁阻止；状态见 [验证记录](docs/UI-TEMPLATES.md)，安全与完整 Maildir 备份要求见 [运维文档](docs/OPERATIONS.md)。
...
第三方客户端必须把用户名填写为完整邮箱地址。SMTP 推荐 `465 + SSL/TLS`，也支持 `587 + STARTTLS`；IMAP 使用 `993 + SSL/TLS`，POP3 使用 `995 + SSL/TLS`。“账号设置 → 通知与客户端”会分开展示可复制的服务器、端口、用户名和简明登录提示：账号未启用 2FA 时三种协议使用邮箱登录密码，启用 2FA 后必须改用该邮箱单独生成的应用密码。同一页面可查看、选择并批量删除最近 90 天的连接成功和鉴权失败记录。
