...
| 管理后台 | 用户与权限组、域名与 DNS 检测、邮件健康评分、邮件收发与账户使用统计、邮箱与别名、邮件审计、发送队列、全域公告、界面模板和系统设置 |
...
更新完成并生成回滚点后，可在同一弹窗点击“回滚上一版本”。回滚前会再次备份数据库并要求二次确认；它只切换镜像和 Compose，不会把数据库恢复到旧时间点。更新器和运维服务都只在 Docker 内部网络开放，不映射公网端口，并使用随机令牌鉴权。普通用户和普通后台权限组无法执行系统更新或回滚。
