...
- 当前界面模板以 `uiTemplate` 保存到 `system_settings`，只允许 `imyemaildefault` 与 `imyemailcloud`。公开设置接口仅下发这个白名单值，React 在根节点设置对应的 `data-ui-template`，由模板样式统一覆盖登录、Webmail、个人中心和后台；切换不复制业务页面，也不改变权限或数据接口。
