...
- 当前界面模板以 `uiTemplate` 保存到 `system_settings`，只允许 `imyemaildefault`、`imyemailcloud` 与 `imyemail-cloud-sy`。公开设置接口仅下发这个白名单值，React 在根节点设置对应的 `data-ui-template`，由模板样式统一覆盖登录、Webmail、个人中心和后台；切换不复制业务页面，也不改变权限或数据接口。`imyemailcloud` 与 `imyemail-cloud-sy` 均使用流式宽度，1440px 以上只扩展导航和邮件列表，不限制整个应用宽度；模板契约检查会验证桌面、宽屏、移动端、浅深色和减少动态效果规则仍然存在。
