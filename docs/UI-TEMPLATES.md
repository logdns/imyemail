...
## v1.3.25 发布候选验证记录（未发布）

2026-09-11：应用测试已通过，已获准采用经校验的 Dovecot 2.4.5 上游构建，候选分支用于原生 amd64/arm64 CI 核验；尚未创建正式标签/Release。下列记录不代表容器安全审计全部通过。具体 Dovecot 风险和修复前提见 [运维说明](OPERATIONS.md)。
...
- Manager 本地 release 构建输出 `imyemail 1.3.25`。本地 all-in-one 构建成功，但仍包含旧 Dovecot 与缓存中的 OpenSSL，不能作为已通过安全审计的发布镜像；必须完成安全修复并重新构建后再发布。
- 使用 `v1.3.25` 固定 CDN 地址独立构建两次，12 个 Web 文件逐字节一致。Gitleaks 扫描当前受版本控制的文件和三个新增源码文件，只有两处 Rspamd 公开签名密钥 SHA-256 校验常量误报；未发现新增密钥泄露。`site` 与本地资产已准备候选版本，但尚未部署，安全门禁通过前不要推送会触发 Pages 的提交。
