...
2026-09-11：候选分支的原生 amd64/arm64 上游测试及真实协议、鉴权、升级回滚曾全部通过，记录见 [候选 CI](https://github.com/logdns/imyemail/actions/runs/34569714342)。主分支复核中，amd64 CPU 压力测试先后触发 5 分钟和 15 分钟上限，最后一轮通过 8/9 个 CPU 子项后超时；[主分支 CI](https://github.com/logdns/imyemail/actions/runs/34571416607) 未通过，正式发布被阻止。其他主分支检查与 arm64 协议回归已通过，正在比较内核 CPU 时钟和资源计数，不跳过测试、不修改运行时源码。Dovecot 来源、数据兼容性和容器剩余风险见 [运维说明](OPERATIONS.md)。
