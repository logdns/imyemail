...
`v1.3.26` 新增此可选功能。进入「后台 → 系统设置 → AI 邮件助手」，选择服务商预设或自定义服务商，填写 API 协议、包含版本路径的公网 HTTPS Base URL、API KEY 和账户可用模型。支持 OpenAI Chat Completions / Responses、Anthropic Messages、Gemini GenerateContent；系统按协议追加操作路径。完整预设、兼容范围与真实测试命令见 [AI 服务商接入](AI-PROVIDERS.md)。先保持关闭并保存，用「测试 AI 连接」发送合成文本测试，成功后再启用并验证写信、总结和回复。测试可能产生少量费用，不读取邮件。
