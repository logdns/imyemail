...
Chrome/Firefox 各 40 组配置页回归：五模板 × 四宽度 × 浅深色，减少动态效果；验证获取、选择、手动模型、未保存测试、修改后清除旧结果、保存与 KEY 保护。三语空列表、加载、错误与 401 诊断另行覆盖。九个 Dockerfile、五组隔离 Compose、Shell/Python/YAML/Pages 检查通过。

真实服务商付费联调仍未执行：没有测试 KEY。合成 transport 和 HTTPS 集成不能证明用户账户、区域、网关或模型已可用；使用方法见 [服务商接入](AI-PROVIDERS.md)。部署到用户服务器后的实际修复效果仍需用其配置验证。

本地邮件镜像构建、容器收发/升级回滚及正式发布资产的最终状态，以后续补充记录和 Release 验证附件为准。回滚到 v1.3.26 前，将根地址或完整接口地址还原为包含版本路径的 Base URL 并重新输入 KEY 保存；无数据库迁移。
